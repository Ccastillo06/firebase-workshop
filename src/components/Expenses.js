...
import { uploadExpenseImage } from '../storage/expenses';

function Expenses({
  sharedExpenseId,
  handleAddImageToExpense,
  expenseList,
  handleSaveExpense,
  participantList,
}) {
  ...
  async function handleUploadImage(e, expenseId) {
    ...
    const imageUrl = await uploadExpenseImage(fileName, file);
    await handleAddImageToExpense(expenseId, imageUrl);
  }
